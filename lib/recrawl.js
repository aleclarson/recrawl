 ...
 */
function createMatcher(globs, mapGlob = (glob) => glob) {
    ...
    globs.forEach(glob => {
        if (globAllRE.test(glob)) {
            ...
            fileGlobs.push(glob_regex_1.default.replace(mapGlob(glob)));
        }
        else {
            nameGlobs.push(glob_regex_1.default.replace(mapGlob(glob)));
        }
    });
